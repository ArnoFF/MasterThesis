...
% Vehicle 3
init.c3_states = [1100 11 0];
init.c3_velocity = 20;
init.c3_omega = 0;
init.c3_ID = 3;
init.c3_mission.v = 20;
init.c3_mission.lane = 11;
...
figure
p = plot(y(1,4),y(1,5),'*g',y(1,7),y(1,8),'*b');
...
xlim([0,590])
set(gcf, 'Color','k')
lgnd = legend(p,'$Veh\:1$', '$Veh\:2$', 'Interpreter', 'latex','FontSize',12);
set(lgnd,'color','k')
...
gif('myfile01.gif')
for k = 2:length(y)
    p = plot(y(k,4),y(k,5),'*g',y(k,7),y(k,8),'*b');
    xlabel('time $t$', 'Interpreter', 'latex')
    title('Position')
    yline(10.5,'-w','LineWidth',1.5)
    yline(11.5,'--w','LineWidth',1.2)
