function [u] = ctrl_main(ego_ID,fleet,t)
% -------------------------------------------------------------------------
% Description:
...
%--------------------------------------------------------------------------
    ...
    cost_fcn = @(xi) xi(1)^2 +40000*xi(2)^2 +1e9*xi(3)^2 +1e9*xi(4)^2;
    
    % Nonlinear constraints: V1,b21,b22,b31,b32
...
%--------------------------------------------------------------------------   
    ...
    if ego_ID==2
    gen.V1(end+1) = eps*0.5*(y_ref-xE(2))^2;
    gen.eta1V1(end+1) = eta1_1(u(1));
    gen.eta2V1(end+1) = eta1_2(u(1),u(2));
    gen.t11(end+1) = -eps*u(1)*sin(psiE)+cV11*(y_ref-xE(2));
    gen.t2(end+1) = eps*(y_ref-xE(2))*u(1)*cos(psiE);
    gen.t12(end+1) = u(1)*sin(psiE);
    gen.t22(end+1) = u(2);
    gen.t3(end+1) = cV12*eta1_1(u(1));
    gen.t111(end+1) = -eps*u(1)*sin(psiE);
    gen.t112(end+1) = cV11*(y_ref-xE(2));
    gen.V1dot(end+1) = -eps*(y_ref-xE(2))*u(1)*sin(psiE);
    gen.alpha(end+1) = cV11*V1;
    gen.gamma(end+1) = cV12*eta1_1(u(1));
    end
end
